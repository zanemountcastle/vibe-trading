...
#[tokio::test]
async fn test_new_crypto_exchange() {
...
#[tokio::test]
async fn test_multiple_market_data_requests() {
    let config = create_test_config();
    let mut exchange = CryptoExchange::new(config);
    ...
    let symbols = vec!["BTC/USD", "ETH/USD", "XRP/USD"];
    
    for symbol in symbols {
        let result = exchange.get_market_data(symbol).await;
        assert!(result.is_ok());
